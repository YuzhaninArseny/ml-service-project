...
def test_register_user(context_user):
    ...
    } or response.json() == {"message": "User already registered!"}
...
def test_deposit(context_user):
    ...
    response = context_user["client"].post(
        "/deposit",
...
def test_get_balance(context_user):
    ...
    response = context_user["client"].get(
        "/balance", cookies={"access_token": context_user["token"]}
    )
    assert response.status_code == 200
...
def test_get_all_users(context_admin):
    ...
    response = context_admin["client"].get(
        "/all_users", cookies={"access_token": context_admin["token"]}
    )
    assert response.status_code == 200
...
def test_get_user_transactions(context_user):
    ...
    response = context_user["client"].get(
        "/all_transactions", cookies={"access_token": context_user["token"]}
    )
    assert response.status_code == 200
    assert isinstance(json.loads(response.json()), list)
...
def test_get_anecdote(context_user):
    ...
    context_user["client"].post(
        "/deposit", json={"amount": 52}, cookies={"access_token": context_user["token"]}
    )

    response = context_user["client"].post(
        "/anecdote",
        json={"prompt": "tell me joke"},
        cookies={"access_token": context_user["token"]},
    )
    ...
    response = context_user["client"].get(
        f"/anecdote/{task_id}", cookies={"access_token": context_user["token"]}
    )
    assert response.status_code == 200
